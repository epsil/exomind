...
class Wiki extends Component {
  ...
  moveToHashOnLoad(hash) {
    ...
    $(function() {
      ...
      const hasDynamicElements = hasImages || hasTables;
  ...
  moveToHash(hash) {
    let hashArg = hash || page.hashArgs(0);
    if (hashArg && hashArg !== '#') {
      const decodedHash = decodeURIComponent(hashArg);
      const latin1Hash = '#' + util.slugify(decodedHash);
  ...
  render() {
    const { prompt, invalidPassword, disabledPrompt, decrypted, markdown } = this.state;
    if (prompt) {
      return <Prompt callback={this.decrypt} invalid={invalidPassword} disabled={disabledPrompt} success={decrypted} />;
    }
    if (!markdown) {
